...
handler = logging.FileHandler(filename="log/bkmkitap.log", mode="w")
handler.setLevel(logging.DEBUG)
handler.setFormatter(formatter)

logger = logging.Logger("bkmkitap")
...
for i in range(1):  # range(50)
    page_link = f"https://www.bkmkitap.com/2024-yks-tyt-ve-ayt-kitaplari?sort=1&stock=1&pg={i+1}"
    ...
    for book in books:
        book_link = "https://www.bkmkitap.com" + book.find("a", attrs={"class": "detailLink"})["href"]
        ...
        sql = "INSERT INTO bkmkitap (name, publisher, number_of_page, current_price, original_price, quantity, score, subject, grade, year, type, link, image) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
...
cursor.execute("SELECT COUNT(*) FROM bkmkitap")
result = cursor.fetchone()
row_count = result[0]

print(f"Completed: bkmkitap ({row_count}/{row_count})")
logger.info(f"{row_count} book has been scraped from bkmkitap.")
